...
int main(int argc, char *argv[])
{
	...
	// TODO find a way to output to stdout conditionally without repeating code
	...
	static std::ofstream Outs(filename, std::ios::out);
	...
	PrintTo(Outs, MakePreProc("ifndef"s, guard), "\n");
	PrintTo(Outs, MakePreProc("define"s, guard), Mult("\n"s, 2));

	// Guard for protection of C++ headers against C code
	if(isCpp && onlyCpp)
	{
		PrintTo(Outs, MakeIfBlock("__cplusplus"s, MakePreProc("error", "This header file is only to be used with C++ code"), 1, true), "\n");
	}
	...
	{
		PrintTo(Outs, MakeIfBlock("__cplusplus"s, "extern \"C\" {"s), "\n");
	}
	...
	PrintTo(Outs, MakeComment("Made by anson in <YEAR>, see LICENSE for related details", true), Mult("\n"s, 4));

	// Ending guard against linking C code with C++
	if(!isCpp || args.at("-c").asBool())
	{
		PrintTo(Outs, MakeIfBlock("__cplusplus"s, "}", 1, false));
	}
